...
export function SVGPlateVisualization({
  ...
  // For custom mode with per-project sheets, each group represents a full sheet
  // We render each project's sheet layout separately
  const isCustomMode = packMode === "custom";

  return (
    <div className="w-full">
      <div className="w-full overflow-x-auto">
        {isCustomMode && placedGroups.length > 1 ? (
          // Per-project sheet rendering for custom mode
          <div className="space-y-4">
            {placedGroups.map((group, gi) => {
              ...
              const sheetCount = group.sheets || alloc?.sheets || 1;
              const sizeLabel = group.sides
                ? `${getPolygonIcon(group.sides)} ${getPolygonName(group.sides)} ${group.stickerWidth}"\u00d7${group.stickerHeight}"`
                : "";

              return (
                <div key={`custom-sheet-${gi}`}>
                  <div className="flex items-center gap-2 mb-1">
                    <div className="w-3 h-3 rounded-sm" style={{ backgroundColor: color }} />
                    <span className="text-sm font-semibold text-slate-200">{group.name}</span>
                    <span className="text-xs text-slate-400">
                      {sizeLabel} — {group.outs} {terms.outs}/{terms.sheet} &times; {sheetCount} {terms.sheet}s = {alloc?.produced.toLocaleString() || (group.outs * sheetCount).toLocaleString()} produced
                    </span>
                  </div>
                  <svg
                  ...
                  >
                    <defs>
                      <marker id={`arrow-${uniqueId}-${gi}`} markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto">
                        <polygon points="0 0, 8 3, 0 6" fill="#06b6d4" />
                      </marker>
                      <pattern id={`bleed-hatch-${uniqueId}-${gi}`} patternUnits="userSpaceOnUse" width="0.15" height="0.15" patternTransform="rotate(45)">
                    ...
                    {/* Full-sheet group boundary */}
                    <rect x={group.x} y={group.y} width={group.width} height={group.height} rx={0.06} fill={color} fillOpacity={0.08} stroke={color} strokeWidth={0.08} />

                    {/* Bleed zones */}
                    {showBleedZone && <rect x={group.x + bleed} y={group.y} width={group.width - 2 * bleed} height={bleed} fill={`url(#bleed-hatch-${uniqueId}-${gi})`} fillOpacity={0.5} />}
                    {showBleedZone && <rect x={group.x + bleed} y={group.y + group.height - bleed} width={group.width - 2 * bleed} height={bleed} fill={`url(#bleed-hatch-${uniqueId}-${gi})`} fillOpacity={0.5} />}
                    {showBleedZone && <rect x={group.x} y={group.y + bleed} width={bleed} height={group.height - 2 * bleed} fill={`url(#bleed-hatch-${uniqueId}-${gi})`} fillOpacity={0.5} />}
                    {showBleedZone && <rect x={group.x + group.width - bleed} y={group.y + bleed} width={bleed} height={group.height - 2 * bleed} fill={`url(#bleed-hatch-${uniqueId}-${gi})`} fillOpacity={0.5} />}

                    {/* Tessellated polygons */}
                    {group.tessellated && group.tessPositions && group.tessPositions.length > 0 && group.vertices && group.stickerWidth && group.stickerHeight && (
                      <>
                    ...
                    <text x={group.x + group.width / 2} y={group.y + group.height / 2 + 0.25} textAnchor="middle" dominantBaseline="middle" fill="#cbd5e1" fontSize={0.28} fontFamily="monospace">{group.outs} {terms.outs} ({group.shape?.w}\u00d7{group.shape?.h})</text>
                    ...
                    {/* Title */}
                    <text x={sheetWidth / 2} y={-1.0} textAnchor="middle" fill="#e2e8f0" fontSize={0.45} fontWeight="bold" fontFamily="monospace">{group.name} — {getPolygonName(group.sides || 4)} Layout</text>
                    <text x={sheetWidth / 2} y={-0.7} textAnchor="middle" fill="#94a3b8" fontSize={0.3} fontFamily="monospace">{sheetCount} {terms.sheet}s &times; {group.outs} {terms.outs}</text>

                    {/* Legend */}
                    <g transform={`translate(0, ${sheetHeight + 1.0})`}>
                      {showBleedZone && (
                        <>
                          <rect x={0} y={0} width={0.3} height={0.2} rx={0.03} fill={`url(#bleed-hatch-${uniqueId}-${gi})`} stroke="#f97316" strokeWidth={0.02} />
                          <text x={0.4} y={0.15} fill="#94a3b8" fontSize={0.2} fontFamily="monospace">{bleedMm}mm {terms.bleed} zone</text>
                        </>
                      )}
          ...
          // Standard rendering for non-custom modes or single-project custom
          <svg
            viewBox={`${-pad} ${-pad} ${svgW} ${svgH}`}
            className="w-full max-w-4xl mx-auto"
            style={{ minHeight: 320 }}
            ...
            {placedGroups.map((group, gi) => {
              ...
              return (
                <g key={`grp-${gi}`}>
                  ...
                  {/* Group labels */}
                  <text x={group.x + group.width / 2} y={group.y + group.height / 2 - 0.2} textAnchor="middle" dominantBaseline="middle" fill={color} fontSize={0.7} fontWeight="bold" fontFamily="monospace">{group.name}</text>
                  <text x={group.x + group.width / 2} y={group.y + group.height / 2 + 0.25} textAnchor="middle" dominantBaseline="middle" fill="#cbd5e1" fontSize={0.28} fontFamily="monospace">{group.outs} {terms.outs} ({shape.w}\u00d7{shape.h})</text>
                  <text x={group.x + group.width / 2} y={group.y + group.height / 2 + 0.5} textAnchor="middle" dominantBaseline="middle" fill="#94a3b8" fontSize={0.2} fontFamily="monospace">{sizeLabel}</text>
