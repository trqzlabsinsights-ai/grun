...
export function SVGPlateVisualization({
  ...
  const customPolygonName = packMode === "custom" && placedGroups.length > 0 && placedGroups[0].sides
    ? getPolygonName(placedGroups[0].sides!)
    : "";
      ...
      <div className="w-full overflow-x-auto">
        ...
        >
          ...
          <text x={sheetWidth / 2} y={-1.0} textAnchor="middle" fill="#e2e8f0" fontSize={0.45} fontWeight="bold" fontFamily="monospace">
            {packMode === "custom" && customPolygonName ? `${customPolygonName} Gang Run` : title}
          </text>
          ...
          <g transform={`translate(0, ${sheetHeight + 1.0})`}>
            ...
            {packMode === "circular" ? (
              ...
              <>
                <polygon points="4,0.12 4.15,0.02 4.3,0.12 4.15,0.22" fill="none" stroke="#f97316" strokeWidth={0.015} strokeDasharray="0.06 0.03" />
                <text x={4.4} y={0.15} fill="#94a3b8" fontSize={0.2} fontFamily="monospace">{cutLineLabel}</text>
              </>
            ) : (
              <>
