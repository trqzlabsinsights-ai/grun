...
function SVGPlateVisualization({
  ...
  const { sheetWidth, sheetHeight, stickerWidth, stickerHeight, bleedInches } = capacity;
  const { placedGroups, allocation, runLength } = plateResult;

  // SVG padding and dimensions
  const pad = 1.2;
  const svgW = sheetWidth + pad * 2;
  const svgH = sheetHeight + pad * 2 + 1.8;

  // Scale factor for good visual sizing
      ...
      <div className="w-full overflow-x-auto">
        ...
        >
          ...
          {placedGroups.map((group, gi) => {
            ...
            const alloc = allocation.find(a => a.name === group.name);
            ...
            return (
              <g key={`grp-${gi}`}>
                ...
                <rect x={group.x + bleedInches} y={group.y} width={group.width - 2 * bleedInches} height={bleedInches} fill={`url(#bleed-hatch-${uniqueId})`} fillOpacity={0.5} />
                {/* Bleed zone — bottom */}
                <rect x={group.x + bleedInches} y={group.y + group.height - bleedInches} width={group.width - 2 * bleedInches} height={bleedInches} fill={`url(#bleed-hatch-${uniqueId})`} fillOpacity={0.5} />
                {/* Bleed zone — left */}
                <rect x={group.x} y={group.y + bleedInches} width={bleedInches} height={group.height - 2 * bleedInches} fill={`url(#bleed-hatch-${uniqueId})`} fillOpacity={0.5} />
                {/* Bleed zone — right */}
                <rect x={group.x + group.width - bleedInches} y={group.y + bleedInches} width={bleedInches} height={group.height - 2 * bleedInches} fill={`url(#bleed-hatch-${uniqueId})`} fillOpacity={0.5} />

                {/* Individual sticker cells within the group */}
                {Array.from({ length: shape.h }).flatMap((_, row) =>
                  Array.from({ length: shape.w }).map((_, col) => {
                    const cellX = group.x + bleedInches + col * stickerWidth;
                    const cellY = group.y + bleedInches + row * stickerHeight;
                    return (
                      <g key={`cell-${gi}-${row}-${col}`}>
                        ...
                        <rect
                          x={cellX + 0.05}
                          y={cellY + 0.05}
                          width={stickerWidth - 0.1}
                          height={stickerHeight - 0.1}
                          rx={0.02}
                          fill="none"
                          stroke="#f97316"
                          strokeWidth={0.02}
      ...
      <header className="border-b border-slate-800 bg-slate-900/80 backdrop-blur-sm sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Group-based optimization with bleed-per-group packing</p>
          </div>
        </div>
      </header>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription className="text-slate-400">
                      Sheet size, sticker size, bleed, and project quantities
                    </CardDescription>
                  </div>
            ...
            <CollapsibleContent>
              <CardContent className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-3">
                    <Label className="text-sm font-semibold text-slate-300">Sticker (inches)</Label>
                    <div className="flex items-center gap-2">
                      <Input type="number" step="0.1" min="0.1" value={stickerWidth} onChange={(e) => setStickerWidth(parseFloat(e.target.value) || 0)} className="bg-slate-800 border-slate-700 text-slate-100" placeholder="W" />
                      <span className="text-slate-500">&times;</span>
                      <Input type="number" step="0.1" min="0.1" value={stickerHeight} onChange={(e) => setStickerHeight(parseFloat(e.target.value) || 0)} className="bg-slate-800 border-slate-700 text-slate-100" placeholder="H" />
                    </div>
                  </div>
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2 min-w-[400px]">
                      {projects.map((project, idx) => (
                        <div key={idx} className="flex items-center gap-2">
                          <div className="w-4 h-4 rounded-sm shrink-0" style={{ backgroundColor: PROJECT_COLORS[idx % PROJECT_COLORS.length] }} />
                          <Input value={project.name} onChange={(e) => updateProject(idx, "name", e.target.value)} className="bg-slate-800 border-slate-700 text-slate-100 w-24" placeholder="Name" />
                          <Input type="number" min="0" value={project.quantity || ""} onChange={(e) => updateProject(idx, "quantity", e.target.value)} className="bg-slate-800 border-slate-700 text-slate-100 flex-1" placeholder="Order Quantity" />
                          <Button variant="ghost" size="sm" onClick={() => removeProject(idx)} className="text-slate-500 hover:text-red-400 hover:bg-red-400/10 shrink-0" disabled={projects.length <= 1}>
                ...
                <div className="flex items-center gap-4 pt-2">
                  ...
                  {loading && <span className="text-sm text-slate-400">Exhaustive search with group packing...</span>}
                </div>
              </CardContent>
            </CollapsibleContent>
          </Card>
            ...
            <Card className="bg-slate-900 border-slate-800">
              ...
              <CardContent>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  <KPICard label="Grid (reference)" value={`${result.capacity.cols} × ${result.capacity.rows}`} accent="text-emerald-400" />
                  <KPICard label="Max Per Sheet" value={result.capacity.maxPerSheet} accent="text-emerald-400" />
                  <KPICard label="Cell Size" value={`${result.capacity.cellWidth.toFixed(3)}" × ${result.capacity.cellHeight.toFixed(3)}"`} sub={`Sticker + ${bleed}mm × 2`} accent="text-slate-300" />
                  <KPICard label="Bleed Per Side" value={`${bleed}mm`} sub={`${result.capacity.bleedInches.toFixed(4)}"`} accent="text-amber-400" />
                </div>
              </CardContent>
            </Card>
              ...
              <TabsContent value="single" className="space-y-6 mt-4">
                {result.singlePlateResult ? (
                  <>
                    ...
                    <Card className="bg-slate-900 border-slate-800">
                      <CardHeader><CardTitle className="text-slate-100 text-base">Production vs Order</CardTitle></CardHeader>
                      <CardContent>
                        <ProductionBarChart allocation={result.singlePlateResult.allocation} projectColors={PROJECT_COLORS} projectNames={projectNames} />
                      </CardContent>
                    </Card>
